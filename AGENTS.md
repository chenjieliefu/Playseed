...
- 两种交付方式共用同一产品：本机（下载到电脑玩）游戏当前可制作，其中 2D 稳定、实验性 3D 房间寻物可用；网页（发链接即玩）游戏制作、3D 能力边界之外的内容、音频、联网和公开发布完成前，要明确显示为尚未接通。
- 生成代码必须先经过静态限制、Godot 解析、隔离运行和重开检查，检查通过后才能成为新版本。
