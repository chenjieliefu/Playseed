"""Open the checked P3 sample without changing the user's project list."""
...
def play():
    ...
    if len(state['steps'])!=4 or any(row['status']!='passed' for row in state['steps']):
        raise ValueError('3D制作与恢复验收尚未完成。')
    project=run/'project/revisions/0004'
    ...
    try: process.wait(timeout=1.2)
        ...
        print('3D温室寻物已打开。WASD移动，靠近按E互动，R重开，Esc暂停。')
        return process.pid
